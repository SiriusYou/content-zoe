...
- Started: 2026-05-29T13:28:15.554Z
- Finished: 2026-05-29T13:28:15.838Z
- Scenario root: /var/folders/77/w_yjdztn54lfvlt0drtcpx040000gn/T/cz-bot-smoke-2026-05-29T13-28-15.553Z (removed by finally-cleanup)
- Result: 74/74 PASS
...
| image-reject-command-parse | PASS | Image /reject parses lexically with image:style_off and preserves the operator reason for modality validation after job lookup. |
| reject-scope-type-matrix | PASS | All text RejectScope x RejectType combinations matched the PLAN matrix.<br>All image RejectScope x RejectType combinations matched the image-only reject matrix. |
...
| image-reject-success-requeues | PASS | Allowed image /reject requeued the image job at generate with attempt+1 and cleared stale notification/summary fields.<br>Image reject reply points operators to content:image-run and exactly one rejected event preserves the old attempt. |
| image-reject-text-combo-invalid | PASS | Image jobs reject text-only scope/type pairs after job lookup and leave DB/events untouched. |
...
| approve-success-publishes-bundle | PASS | Allowed approve staged and atomically published reports, research, sources, and source-material into reports/2026-W47-ai-trends.<br>DB row is published with preserved report metadata and exactly one promoted event carrying the authoritative publish_manifest with source-material/manifest.json.<br>Only the approved attempt was cleaned up and fake git received a path-bounded plan. |
| image-approve-deferred | PASS | Image /approve fails closed as IMAGE_PUBLISH_NOT_IMPLEMENTED with no DB/event/filesystem mutation.<br>Image approve deferral does not call promoteJob's committer path. |
| approve-without-source-material-optional | PASS | Attempt without source-material/ still publishes successfully.<br>The promoted manifest and final reports directory contain no source-material entries when staging is absent. |
| approve-missing-sources-json | PASS | Valid cloned attempt with sources.json removed fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-malformed-sources-json | PASS | Valid cloned attempt with malformed sources.json fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-invalid-sources-provenance | PASS | Valid cloned attempt with sources.json citing run-state.json fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-missing-report-en | PASS | Valid cloned attempt with report.en.md removed fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-source-material-present-missing-manifest | PASS | Valid cloned attempt with source-material/ present but manifest.json removed fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-source-material-manifest-shape-invalid | PASS | Valid cloned attempt with shape-invalid source-material manifest fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-source-material-operator-hash-mismatch | PASS | Valid cloned attempt with operator_source sha256 mismatch fails before reports/DB/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
| approve-status-attempt-mismatch | PASS | Attempt mismatch fails before source/files/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt.<br>Status mismatch fails before source/files/events mutation.<br>The failed approve left the job row unchanged, wrote no events.promoted, created no final reports dir, and preserved the source attempt. |
...
| approve-git-commit-failure-nonblocking | PASS | Fake git committer failure was non-blocking: job stayed published and promoted event remained authoritative with source-material files.<br>A git_commit_failed event captured diagnostics and fake plan assertions proved path-bounded argv semantics. |
...
| bot-db-path-cwd | PASS | Default DB path resolved to /var/folders/77/w_yjdztn54lfvlt0drtcpx040000gn/T/cz-bot-smoke-2026-05-29T13-28-15.553Z/bot-db-path-cwd/runtime-cwd/.data/content.db.<br>Default tick interval remains 10000. |
