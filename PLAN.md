...
[x] (cz-Claude approved 2026-05-28; cz-Codex approved 2026-05-28; classification=framework) V2 Slice 1: modality registry + image manifest rule kinds -- file scope: src/pipeline/types.ts, src/pipeline/run-stage.ts, src/pipeline/modality.ts (NEW), src/pipeline/stages.ts, src/lib/report-run-fake-provider.ts, scripts/manifest-image-rules-smoke.ts (NEW), scripts/modality-registry-smoke.ts (NEW), docs/preflight/manifest-image-rules-smoke.md (NEW), docs/preflight/modality-registry-smoke.md (NEW), package.json (scripts only). Dependency satisfied: V2 Slice 0.5a + 0.5b merged at `0a4509d7`; `bun run typecheck` is live and must remain green. Hard out: src/pipeline/image/*, src/pipeline/research.ts, src/pipeline/draft-en.ts, src/pipeline/edit-en.ts, src/pipeline/translate-zh.ts, src/llm/* except the named fake-provider file, src/db.ts, src/migrations/*, src/lib/report-loop.ts, src/lib/runtime-config.ts, src/telegram/*, real provider execution, image stage implementations, DB modality schema, CLI/reject/publish surfaces, and all governance/process docs. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-1-modality-registry-v1.1.md` SHA `d82de38a3a167719c0cf28bed2778de6e82464b069b2c3fe149b8f349771b7aa`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-1-review-2026-05-28-r2.md` SHA `edb9d20b94c9bbeedfd9b8b81acb1620c55c1c5a239d7a2a3920f4e76e29a2f9`; cz-Codex r2 `.omx/artifacts/codex-slice-v2-1-review-2026-05-28-r2.md` SHA `d22f0d66e41fc6d768d4329a31bf070d57ee5b631974abdffe2f93cb6e615486`.

[x] (cz-Claude approved 2026-05-28; cz-Codex approved 2026-05-28; classification=framework) V2 Slice 2: image-spec + judge-verdict contracts -- file scope: src/pipeline/image/spec.ts (NEW), src/pipeline/image/verdict.ts (NEW), scripts/image-contracts-smoke.ts (NEW), docs/preflight/image-contracts-smoke.md (NEW), package.json (scripts only: image-contracts-smoke). Dependency satisfied: V2 Slice 1 merged at `8c8d5534`; `bun run typecheck` is live and must remain green. Contract constraints: image dimensions are restricted to `1024x1024`, `1536x1024`, or `1024x1536`; `JudgeVerdict.overallPass` must match all criterion pass values; failed verdicts require non-empty `regenerateFeedback`; smoke coverage must implement all 27 required scenarios in the approved draft. Slice 6 carry-forward: before any image judge stage may use the existing `judge_verdict_pass` manifest rule, reopen `src/pipeline/run-stage.ts` and wire that rule to `parseJudgeVerdict`. Hard out: src/pipeline/types.ts, src/pipeline/run-stage.ts, src/pipeline/modality.ts, src/pipeline/stages.ts, src/pipeline/image/elaborate-spec.ts, src/pipeline/image/generate.ts, src/pipeline/image/judge.ts, src/llm/*, src/db.ts, src/migrations/*, src/bin/*, src/lib/report-loop.ts, src/lib/runtime-config.ts, src/telegram/*, real provider execution, image stage implementations, DB modality schema, CLI/reject/publish surfaces, and all governance/process docs. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-2-image-contracts-v1.1.md` SHA `e2f99d3583d2009eb43c16ed782aba1e2ee061eb8de59f815c98c1b0c9462a92`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-2-review-r2-2026-05-28.md` SHA `13c0204f082c2be20f19d3927391dc6e2c5d5ae744ec1778137d51b39f744f3f`; cz-Codex r2 `.omx/artifacts/codex-slice-v2-2-review-r2-2026-05-28.md` SHA `871b1fdddcfbfbe701a9c96af9ea61defd06ec72c1d923bcc806c5764cfd07f2`.

[x] (cz-Claude approved 2026-05-28; cz-Codex approved 2026-05-28; classification=framework) V2 Slice 3: DB modality column + image event vocabulary -- file scope: src/migrations/0003_jobs_modality.sql (NEW), src/db.ts, scripts/db-modality-smoke.ts (NEW), docs/preflight/db-modality-smoke.md (NEW), package.json (scripts only: db-modality-smoke). Dependency satisfied: V2 Slice 2 merged at `f61e5b2`; `bun run typecheck` is live and must remain green. Hard out: src/migrations/0001_initial.sql, src/migrations/0002_jobs_purpose.sql, scripts/db-smoke.ts, docs/preflight/db-smoke.md (transient `db:smoke` regeneration only; restore before handback), src/pipeline/*, src/llm/*, src/bin/*, src/lib/report-loop.ts, src/lib/runtime-config.ts, src/telegram/*, src/promote.ts, approval/reject/publish surfaces, real provider execution, image stage implementations, CLI grammar, and governance/process docs. Safety invariant: this slice must not create any code path that advances image jobs to `awaiting_approval` or exposes them to existing text Telegram approval/publish surfaces; retask if enforcing that requires OUT-scope edits. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-3-db-modality-v1.1.md` SHA `b79c07b77b21016207f4cf3c9dd4a65bd1cb2d7983057d99d0e8f4b49d28e27b`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-3-review-2026-05-28-r2.md` SHA `9a90d2c1c75fe695d58a4e7b9c4ff7c607eed9f5ea347b838537053e61713011`; cz-Codex r2 `.omx/artifacts/codex-slice-v2-3-review-2026-05-28-r2.md` SHA `8f1996eb9363c683ac53db9fed72ea5263867c262c254f816b8ae3f1bf33a03a`.

[x] (cz-Claude approved 2026-05-28; cz-Codex approved 2026-05-28; classification=framework) V2 Slice 4: image provider interface + OpenAI + fake -- file scope: src/llm/image-provider.ts (NEW), src/llm/image-openai.ts (NEW), src/llm/image-fake.ts (NEW), scripts/image-provider-fake-smoke.ts (NEW), docs/preflight/image-provider-fake-smoke.md (NEW), package.json (scripts only: image-provider-fake-smoke). Dependency satisfied: V2 Slice 3 merged at `64deab8`; `bun run typecheck` is live and must remain green. Hard out: src/pipeline/* except read-only imports from src/pipeline/image/spec.ts, src/pipeline/types.ts, and src/pipeline/run-stage.ts; src/bin/*; src/db.ts; src/migrations/*; src/lib/report-loop.ts; src/lib/runtime-config.ts; src/telegram/*; src/promote.ts; approval/reject/publish surfaces; real provider execution; image stage implementations; DB modality schema; CLI grammar; and governance/process docs. Provider contract: OpenAI image generation is constructor-injected and may not read `process.env`; `gpt-image-1`/non-DALL-E requests omit `response_format`, DALL-E requests use `response_format: "b64_json"`, and only `data[0].b64_json` is accepted. Fake provider must write deterministic complete PNG bytes with non-empty zlib IDAT and valid IHDR dimensions, record feedback/calls, and support failure injection. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-4-image-provider-v1.4.md` SHA `11377f8ffea9407d605ab5f9f8632b9475813db454c2bedd8d73c53317e99e02`; cz-Claude r4 `.omx/artifacts/claude-slice-v2-4-review-2026-05-28-r4.md` SHA `ace2c932c0de1493adafed02e29a3b9bc62f67e03254d50fc572dd4ba53b25c3`; cz-Codex r4 `.omx/artifacts/codex-slice-v2-4-review-2026-05-28-r4.md` SHA `04fd6e860dff5691d7c3127ddcf230e1e8ed3ab099df020d92292b5812d8b5a2`.

[x] (cz-Claude approved 2026-05-28; cz-Codex approved 2026-05-28; classification=framework) V2 Slice 5: vision judge interface + OpenAI + fake -- file scope: src/llm/vision-judge.ts (NEW), src/llm/vision-judge-openai.ts (NEW), src/llm/vision-judge-fake.ts (NEW), scripts/vision-judge-fake-smoke.ts (NEW), docs/preflight/vision-judge-fake-smoke.md (NEW), package.json (scripts only: vision-judge-fake-smoke). Dependency satisfied: V2 Slice 4 merged at `ba1b2253`; `bun run typecheck` is live and must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `ba1b22531a9e878cca7812ec28ed5abca7587531`. Hard out: src/pipeline/* except read-only imports from src/pipeline/image/spec.ts and src/pipeline/image/verdict.ts; src/bin/*; src/db.ts; src/migrations/*; src/lib/report-loop.ts; src/lib/runtime-config.ts; src/telegram/*; src/promote.ts; approval/reject/publish surfaces; DB modality schema/lifecycle state; image stage implementations; CLI grammar; real provider execution; new runtime dependencies or OpenAI SDK packages; and governance/process docs. Provider contract: OpenAI vision judge is constructor-injected and may not read `process.env`; no SDK; reads PNG bytes into a `data:image/png;base64,...` chat-completions image URL; sends JSON-mode request with all spec criteria; tolerates plain or fenced JSON; validates `parseJudgeVerdict` plus exact criterion-id set and cardinality; maps `content_filter`, non-empty `message.refusal`, and readable safety/refusal bodies to `safety`; maps unreadable non-2xx to `http`; maps image-read/malformed output/criterion mismatch to `parse`. Fake judge uses an explicit scripted verdict queue, records calls, deep-clones returns, supports failure injection, and has no implicit passing default. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-5-vision-judge-v1.1.md` SHA `2a02bf0b282eda6311b3ec8e75267bf99b22fd5bff3d10c518565336b14cbc19`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-5-review-2026-05-28-r2.md` SHA `536505e4e8af1306417b46f135cd88954edb3ead2484465d85791820dc0aa020`; cz-Codex r2 `.omx/artifacts/codex-slice-v2-5-review-2026-05-28-r2.md` SHA `3a9c9c93802a9fdd7c78466abd55c5daf333f5307afa48e81577afe2d405163b`. Carry-forward: Slice 6 must keep mechanical image manifest checks authoritative and ensure `verdict.json` entering `judge_verdict_pass` comes from the provider path.

[x] (cz-Claude approved 2026-05-28 via r2 product-contract review; cz-Codex approved 2026-05-28 via r3 v1.3 acceptance-scope review; classification=framework) V2 Slice 6a: executor seam (modality-agnostic StageDef.run) -- file scope: src/pipeline/types.ts, src/pipeline/run-stage.ts, scripts/executor-seam-smoke.ts (NEW), docs/preflight/executor-seam-smoke.md (NEW), package.json (scripts only: executor-seam-smoke). Dependency satisfied: V2 Slice 5 merged at `d701e7a`; `bun run typecheck` is live and must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `d701e7a0f9168e0c04ddfdd1f9bf69a9cec7287d`. Contract constraints: `StageRunContext.runDir` is the canonical realpath run directory; `timeoutMs` is copied from `StageDef`; run-handler success returns `output: ""`; run-handler throws, missing non-run provider, and manifest-validator throws reuse the existing `StageResult.error` / `LLMProviderError` shape; a run-handler branch bypasses `buildPrompt`; text stages still use the existing provider `runPrompt` fallback; no image/judge imports or references enter shared executor files. Hard out: src/pipeline/modality.ts, src/pipeline/image/*, src/llm/* except the existing text-provider import in src/pipeline/run-stage.ts, src/lib/report-loop.ts, src/db.ts, src/migrations/*, src/bin/*, src/telegram/*, src/promote.ts, approval/reject/publish/gallery surfaces, runtime config, image stage implementations, provider injection plumbing, DB schema/lifecycle state, CLI grammar, real provider execution, new runtime dependencies, and governance/process docs. Acceptance excludes image-provider-fake-smoke and vision-judge-fake-smoke because their static boundary checks are prior-slice-local; Slice 6a proves no image/judge leakage with executor-seam-no-image-imports and preserves V1 behavior with runnable text/core smokes. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-6a-executor-seam-v1.3.md` SHA `8c8df2ef7341ecd7b1a2e4c94b63635c8ff82143bbdb6ad4c6214f3d3f6d1a70`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-6a-review-2026-05-28-r2.md` SHA `704812d65bdb464a238f2ebf1eb3dfb7e7c060096e1f2f710c92a9ae300f3b99`; cz-Codex r3 `.omx/artifacts/codex-slice-v2-6a-review-2026-05-28-r3.md` SHA `baa7895c3a0adae58e9eb55fbbf275919e528ddd7d22aeb4121577436df7b527`.

[x] (operator-directed 2026-05-28 after Slice 6a origin-confirmed; classification=framework) V2 Slice 6b: image stages + regenerate loop -- file scope: src/pipeline/image/elaborate-spec.ts (NEW), src/pipeline/image/generate.ts (NEW), src/pipeline/image/judge.ts (NEW), src/pipeline/modality.ts, src/pipeline/run-stage.ts (judge_verdict_pass only), src/lib/report-loop.ts, src/lib/image-run-fake-provider.ts (NEW), scripts/image-pipeline-smoke.ts (NEW), docs/preflight/image-pipeline-smoke.md (NEW), package.json (scripts only: image-pipeline-smoke). Dependency satisfied: V2 Slice 6a merged at `87918be`; `bun run typecheck`, `bun run preflight`, and `bun run executor-seam-smoke` are live and must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `87918bea638e7fcdfbadc5fdfaad9e30d0c685dd`. Contract constraints: image stages use the 6a `StageDef.run` seam; `elaborate_spec` remains a text stage; image provider and vision judge reach stage defs through factory/deps wiring, not shared executor types; `judge_verdict_pass` must use `parseJudgeVerdict`; mechanical image manifest checks remain authoritative; only parseable `overallPass:false` drives bounded regen; safety escalates to `awaiting_approval` without regen; non-safety provider/judge throws remain ordinary stage failures; regen state persists in `run-state.json`. Hard out: src/db.ts, src/migrations/*, src/bin/*, src/telegram/*, src/promote.ts, approval/reject/publish/gallery surfaces, runtime config, real provider execution, new runtime dependencies, and governance/process docs. Approved draft basis `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-6b-image-stages-regen-v1.0.md` SHA `8b42e62dafb60ebb6e537d08992028d1f098da8ed65cfaad3d126d191176636a`; implementation resolves the draft's provider-plumbing, dynamic-dimension, safety-criterion, and loop-hook `[VERIFY]` items in code and smoke evidence.

[x] (cz-Claude approved 2026-05-29 via r2; cz-Codex approved 2026-05-29 via r3; classification=framework) V2 Slice 7a: modality-aware Telegram control -- file scope: src/pipeline/modality.ts, src/telegram/commands.ts, src/lib/runtime-config.ts, scripts/bot-smoke.ts, docs/preflight/bot-smoke.md, scripts/report-run-smoke.ts, docs/preflight/report-run-smoke.md. Dependency satisfied: V2 Slice 6b merged at `8ddc8a9`; `bun run typecheck`, `bun run preflight`, `bun run bot-smoke`, and `bun run report-run-smoke` must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `8ddc8a9e615e56aa18ffc1d5d9d8dbeefdd3a70c`. Contract constraints: `modality.ts` owns runtime reject vocabulary and modality-scoped validators; `commands.ts` may not retain local reject scope/type/matrix truth; text reject/approve behavior remains unchanged; image reject validates `image:subject_off|style_off|composition_off|safety`, rewinds to `generate`, and replies with `content:image-run`; image approve fails closed as `IMAGE_PUBLISH_NOT_IMPLEMENTED` with no mutation or promote call; `report-run.ts` remains text-only; image runtime provider parsing must not make text `loadRuntimeConfig()` read or fail on image-only env vars. Hard out: src/bin/content-image-create.ts, src/bin/content-image-run.ts, src/bin/content-image-show.ts, src/bin/report-run.ts, src/promote.ts, src/db.ts, src/migrations/*, src/lib/report-loop.ts, src/pipeline/run-stage.ts, src/pipeline/types.ts, src/pipeline/image/*, src/llm/*, image publish/gallery/README mutation, real provider execution, new runtime dependencies, and governance/process docs. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-7a-telegram-modality-control-v1.2.md` SHA `8dd10e28587bc17b49cabb983f5ff744c44ed94a35fee52aaa120e7c8589a0c0`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-7a-review-2026-05-29-r2.md` SHA `1b11efe018dfcde2d82a8495acbda2202f6682c787305a33722df455c259c5cf`; cz-Codex r3 `.omx/artifacts/codex-slice-v2-7a-review-2026-05-29-r3.md` SHA `af50d12a2295be6f3526b9e928ee68d917eba316ca4006f908c932b47577b115`.

[x] (cz-Claude approved 2026-05-29; cz-Codex approved 2026-05-29 via r3; classification=framework) V2 Slice 7b: image CLI create/run/show -- file scope: src/bin/content-image-create.ts (NEW), src/bin/content-image-run.ts (NEW), src/bin/content-image-show.ts (NEW), scripts/content-image-cli-smoke.ts (NEW), docs/preflight/content-image-cli-smoke.md (NEW), package.json (scripts only: content:image-create, content:image-run, content:image-show, content-image-cli-smoke), src/lib/runtime-config.ts (bounded reopen only if needed for CLI image-provider parsing), src/telegram/commands.ts and src/pipeline/modality.ts (read-only unless 7a was not merged). Dependency satisfied: V2 Slice 7a merged and origin-confirmed at `c4bc54910dfcae9156ac267f933dae148f9d9506`; `bun run typecheck`, `bun run preflight`, `bun run image-pipeline-smoke`, and `bun run bot-smoke` must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `c4bc54910dfcae9156ac267f933dae148f9d9506`. Contract constraints: `content:image-create` creates deterministic `img-<key>` image jobs and writes `request.txt`; `content:image-run` uses fake providers for worker smokes, fails closed on mixed fake/real provider modes before provider construction or filesystem mutation, writes image lifecycle events with exact payload keys, carries `request.txt`/`spec.json`/`image.png` forward for reject rewinds as needed, and persists a text-only `approval_summary` while leaving report-specific artifact columns null; `--resume` returns `IMAGE_RESUME_NOT_IMPLEMENTED` with no mutation, no new attempt directory, and no provider construction; `content:image-show` prints request/spec/verdict text and image metadata without dumping PNG bytes; image approve/publish remains deferred to Slice 8. Hard out: src/bin/report-run.ts, src/promote.ts, src/db.ts, src/migrations/*, src/lib/report-loop.ts, src/pipeline/run-stage.ts, src/pipeline/types.ts, src/pipeline/image/*, src/llm/* except read-only imports/constructor use from existing providers, image publish/gallery/README mutation, real provider execution, new runtime dependencies, and governance/process docs. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-7b-image-cli-v1.3.md` SHA `69435a2308266838924be22236b7114351956ad864efb106d350720ac6ca0dc1`; cz-Claude `.omx/artifacts/claude-slice-v2-7b-review-2026-05-29.md` SHA `3440b6d2066a06b00a614160262313e657ae03c2dc998af211bbe848b46076d9`; cz-Codex r3 `.omx/artifacts/codex-slice-v2-7b-review-2026-05-29-r3.md` SHA `ef6c394800ae402d812ece7c5d5ad35dc92eda8ecc885543c52992f8f0302fa7`.

[x] (cz-Claude approved 2026-05-29 via r2; cz-Codex approved 2026-05-29 via r2; classification=framework) V2 Slice 8: image publish + gallery index -- file scope: src/promote.ts, src/lib/readme-image-gallery-destination.ts (NEW), src/telegram/commands.ts, scripts/image-publish-smoke.ts (NEW), docs/preflight/image-publish-smoke.md (NEW), scripts/bot-smoke.ts, docs/preflight/bot-smoke.md, package.json (scripts only: image-publish-smoke). Dependency satisfied: V2 Slice 7b merged and origin-confirmed at `24fcb93eb5703ca60c4f22bbdf3ff85d074145e2`; `bun run typecheck`, `bun run preflight`, `bun run content-image-cli-smoke`, and `bun run bot-smoke` must remain green. Implementation hard-out anchor: this approval-label commit as the worker branch base; dependency base: `24fcb93eb5703ca60c4f22bbdf3ff85d074145e2`. Contract constraints: image `/approve` replaces the Slice 7a fail-closed deferral with `promoteImageJob`, validates exactly `request.txt`, `spec.json`, `image.png`, and `verdict.json`, requires authoritative image spec/verdict parsing, criterion-id alignment, regular-file/non-symlink inputs, PNG dimensions matching the spec, `verdict.overallPass === true`, and current job `purpose` of `production` or `validation`; publishes exactly those four files under `images/<jobId>/`; records a `promoted` event with the existing `PublishManifest` shape; updates a separate README image-gallery region for production jobs; excludes/removes validation rows; records `image_gallery_update_failed` without rolling back DB publication on post-commit gallery failure and retries self-heal on idempotent re-approve; production git plans include `images/<jobId>/` and `README.md`, validation plans include only `images/<jobId>/`; text report approval/publish behavior and the existing report README managed region remain unchanged. Hard out: src/bin/report-run.ts, src/bin/content-image-run.ts, src/bin/content-image-create.ts, src/bin/content-image-show.ts, src/db.ts, src/migrations/*, src/lib/report-loop.ts, src/pipeline/*, src/llm/*, src/lib/runtime-config.ts, text report bundle schema, report README CLI behavior, real provider execution, `bun run report:run`, real-provider `bun run content:image-run`, new runtime dependencies, and governance/process docs beyond this approval label. Approved draft `file:///Users/youjia/dev/openclaw-healthcare/.omx/drafts/slice-v2-8-image-publish-gallery-v1.1.md` SHA `bda10c785382290acb4d28745a32f96e09d8c18fcad340b97f2d27989420261a`; cz-Claude r2 `.omx/artifacts/claude-slice-v2-8-review-2026-05-29-r2.md` SHA `366bde6e61d691772707a2e4261c443a2d2756870e1fce6e333fdfe8c1f9f4bc`; cz-Codex r2 `.omx/artifacts/codex-slice-v2-8-review-2026-05-29-r2.md` SHA `1bfd439b2363ca3970a185641aeb1e3d9a7fed90dba1c0ad6616bb094b0b5c8c`.

[x] (operator-directed 2026-05-30; classification=framework) V2 provider extension: Google-primary image generation + Google vision judge with explicit OpenAI fallback -- file scope: src/llm/image-google.ts (NEW), src/llm/vision-judge-google.ts (NEW), src/llm/provider-fallback.ts (NEW), src/lib/runtime-config.ts, src/bin/content-image-run.ts, scripts/image-provider-fake-smoke.ts, docs/preflight/image-provider-fake-smoke.md, scripts/vision-judge-fake-smoke.ts, docs/preflight/vision-judge-fake-smoke.md, scripts/content-image-cli-smoke.ts, docs/preflight/content-image-cli-smoke.md. Dependency satisfied: V2 Slice 8 merged and origin-confirmed at `9361f84`, with follow-up smoke-boundary fix `8db9089`; `bun run typecheck`, `bun run preflight`, `bun run image-provider-fake-smoke`, `bun run vision-judge-fake-smoke`, and `bun run content-image-cli-smoke` must remain green. Contract constraints: `IMAGE_PROVIDER=google` and `VISION_JUDGE_PROVIDER=google` are valid real-provider modes with `LLM_PROVIDER=codex`; Google providers use constructor-injected `GOOGLE_API_KEY` and fetch-based Gemini `generateContent` calls with no Google SDK dependency or provider-local env reads; `IMAGE_MODEL=nano-banana-2` and `IMAGE_MODEL="nano banana 2"` normalize to `gemini-3.1-flash-image`; `VISION_JUDGE_MODEL=gemini3.1-pro` normalizes to `gemini-3.1-pro`; OpenAI fallback is opt-in through `IMAGE_PROVIDER_FALLBACK=openai` and/or `VISION_JUDGE_PROVIDER_FALLBACK=openai`, requires `OPENAI_API_KEY`, logs an explicit provider-fallback line, and never falls back on safety-class failures. Hard out: real provider execution, `bun run report:run`, real-provider `bun run content:image-run`, DB schema/migrations, publish/gallery semantics, report pipeline behavior, Telegram approval semantics, package dependencies/SDKs, and source prompts/stage logic outside constructor use of existing provider interfaces.
